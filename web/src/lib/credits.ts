...
type GrantOpts = { jobId?: string; stripeEventId?: string };
  ...
  return db.transaction(async (tx) => {
    if (opts.stripeEventId) {
      const inserted = await tx
        .insert(creditLedger)
        .values({ userId, delta, reason, stripeEventId: opts.stripeEventId })
        .onConflictDoNothing({ target: creditLedger.stripeEventId })
        .returning({ id: creditLedger.id });
      if (inserted.length === 0) return false;
    } else {
      await tx.insert(creditLedger).values({ userId, delta, reason, jobId: opts.jobId });
    }
    await tx
      .update(users)
